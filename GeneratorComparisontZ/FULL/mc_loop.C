 ...
 TH1F *testNjets=new TH1F("testNjets","testNjets",10,0,10);
  ...
  for(int q=0;q<nEvents;q++)
	...
	cout<<q<<'\n';
      ...
      int nparticles = (event->mc_n);
      cout<<"hey"<<'\n';
      cout<<nparticles<<'\n';
      for(int i=0;i<nparticles;i++)//particle loop fills basic histograms
	{
	  int PID=(*event->mc_pdgId).at(i);
	  double pt=(*event->mc_pt).at(i);
	  double eta=(*event->mc_eta).at(i);
	  double phi=(*event->mc_phi).at(i);
	  double E=(*event->mc_E).at(i);
	  int status=(*event->mc_status).at(i);
	  ...
	  if(goodevent)
	    {
	      ...
	      //cout<<"zLeptonPt"<<zLepton.Pt()/1000<<'\n';
	      //cout<<"zAntiLeptonPt"<<zAntiLepton.Pt()/1000<<'\n';
	      ...
	      WHel->Fill(WHelicity/1000,theweight);
	      h_hist->Fill(h/1000,theweight);
	      ht_hist->Fill(ht/1000,theweight);
