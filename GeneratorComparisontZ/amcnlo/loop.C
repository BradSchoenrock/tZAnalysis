...
void loop()
{
  ...
  TFile *rootfile=new TFile("/home/schoenr1/work/GeneratorComparisontZ/amcnlo/unweighted_events.root");
		  ...
		  if(ltype.at(u)==1)
		    {
		      ...
		      if(!first)
			{
			  zLepton1=lepton.at(u);
			  first=true;
			}
		      else
			zLepton2=lepton.at(u);
  ...
  TFile outfile("histos-truth-amcnlo.root","recreate");
