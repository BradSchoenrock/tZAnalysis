...
void SHORTLOOP()
{
  ...
  for(int q=0;q<nEvents;q++)
      ...
      for(int e=0;e<event->Jet_size;e++)
	{
      ...
      if(lepton.size()>0 && eventSel)
	{
	  //cout<<"size="<<lepton.size()<<'\n';
	  for(int i=0;i<lepton.size()-1;i++)
	    {
	      //for(int q=0;q<lepton.size();q++)
	      //cout<<lepton.at(q).Pt()<<"   ";
	      double maxpt=0;
	      int place=-1;
	      for(unsigned int v=i;v<lepton.size();v++)
		{
	      ...
	      //cout<<"i="<<i<<"place="<<place<<'\n';
	      assert(place>=i);
	      if(place!=i)
		{
		  TLorentzVector temp=lepton.at(place);
		  lepton.at(place)=lepton.at(i);
		  lepton.at(i)=temp;
		  int tempi=lcharge.at(place);
		  lcharge.at(place)=lcharge.at(i);
		  lcharge.at(i)=tempi;
		  int tempt=ltype.at(place);
		  ltype.at(place)=ltype.at(i);
		  ltype.at(i)=tempt;
		}
	    }
	}
      ...
      if(eventSel)
	{
	  ...
	  double WTransverseMass=1;
	  int nbjets=0;
	  if(jet.size()==2 && isb.at(0)==true)nbjets++;
	  if(jet.size()==2 && isb.at(1)==true)nbjets++;
	  ...
	  if(lepton.size()!=3)
	    {
	      ...
	      WTransverseMass=Wvariables::WtransverseMass(leftoverLepton.Pt(),leftoverLepton.Phi(),jMET,METPhi);
