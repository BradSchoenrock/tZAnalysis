...
// Thu Nov 14 16:24:08 2013 by ROOT version 5.34/03
// from TTree mini/4-vectors + variables required for scaling factors
// found on file: ML_LOOSEELE_LOOSEMU_41D8_WWW.root
//////////////////////////////////////////////////////////
...
public :
   ...
   Float_t         lep_trackd0pvunbiased[3];   //[lep_n]
   Float_t         lep_tracksigd0pvunbiased[3];   //[lep_n]
   ...
   Float_t         jet_MV1c[10];   //[alljet_n]
   UInt_t          jet_flag[10];   //[alljet_n]
...
#ifdef TopD3PD_cxx
TopD3PD::TopD3PD(TTree *tree) : fChain(0) 
{
// if parameter tree is not specified (or zero), connect the file
// used to generate this class and read the Tree.
   if (tree == 0) {
      TFile *f = (TFile*)gROOT->GetListOfFiles()->FindObject("ML_LOOSEELE_LOOSEMU_41D8_WWW.root");
      if (!f || !f->IsOpen()) {
         f = new TFile("ML_LOOSEELE_LOOSEMU_41D8_WWW.root");
      }
...
Int_t TopD3PD::GetEntry(Long64_t entry)
{
// Read contents of entry.
   if (!fChain) return 0;
   return fChain->GetEntry(entry);
}
Long64_t TopD3PD::LoadTree(Long64_t entry)
{
...
void TopD3PD::Show(Long64_t entry)
{
...
Int_t TopD3PD::Cut(Long64_t entry)
{
